...
async def test_fetch_slots_returns_all_slots_when_none_booked(db):
    result = await fetch_slots(db, "2030-10-15")
    
    assert result["date"] == "2030-10-15"
...
async def test_fetch_slots_excludes_booked_slots(db):
    ...
    await create_appointment(db, user["id"], "2030-10-15", "14:30")
...
async def test_book_appointment_returns_confirmation(db):
    ...
    result = await book_appointment(db, user["id"], "2030-10-15", "10:00")
...
async def test_retrieve_appointments_returns_user_bookings(db):
    ...
    await book_appointment(db, user["id"], "2030-10-16", "14:00")
    
    result = await retrieve_appointments(db, user["id"])
    
    assert len(result) == 2
    assert result[0]["date"] == "2030-10-15"
    assert result[1]["date"] == "2030-10-16"

async def test_cancel_appointment_frees_slot(db):
    user = await identify_user(db, "+1234567890", "John Doe")
    appt = await book_appointment(db, user["id"], "2030-10-15", "10:00")
    ...
    slots = await fetch_slots(db, "2030-10-15")
